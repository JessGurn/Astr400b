...
# and simulation snapshot
    ...
    def COM_P(self, delta):
        ...
        # all magnitutde is sqrt(x^2 + y^2 + z^2)
...
if __name__ == '__main__' : 
    ...
    print('Mily Way COM Position:', MW_COM_p)
    ...
    # Compute the separation between MW and M31
    MW_M31_sep = np.sqrt(np.sum((MW_COM_p - M31_COM_p) ** 2))
    MW_M31_vel = np.sqrt(np.sum((MW_COM_v - M31_COM_v) ** 2))

    # Compute the separation between M33 and M31
    M33_M31_sep = np.sqrt(np.sum((M33_COM_p - M31_COM_p) ** 2))
    M33_M31_vel = np.sqrt(np.sum((M33_COM_v - M31_COM_v) ** 2))

    # Print results rounded to three decimal places
    print(f"MW-M31 Separation: {MW_M31_sep:.3f}")
    print(f"MW-M31 Relative Velocity: {MW_M31_vel:.3f}\n")

    print(f"M33-M31 Separation: {M33_M31_sep:.3f}")
    print(f"M33-M31 Relative Velocity: {M33_M31_vel:.3f}\n")

    # Question 4: Why is the iterative process important?
    print("\nWhy is the iterative COM calculation important?")
    print('''
